...
void test6() {
    ...
    // bool succeeded = q.try_enqueue(18);  // Will only succeed if the queue
    // has an
...
void test10() {
    ...
    for (int i = 0; i != ConsumerCount; ++i) {
        consumers[i] = std::thread([&]() {
            ...
            bool itemsLeft;
            do {
                // It's important to fence (if the producers have finished)
                // *before* dequeueing
                fmt::print("doneConsumers: {}\n", doneConsumers.load());
                itemsLeft = doneProducers.load(std::memory_order_acquire) !=
                            ProducerCount;
                while (q.try_dequeue(item)) {
                    itemsLeft = true;
                    // fmt::print("item: {}\n", item);
                     ...
                     doneConsumers.fetch_add(1, std::memory_order_acq_rel) +
                             1 ==
                         ConsumerCount);
            // The condition above is a bit tricky, but it's necessary to ensure
            // that the last consumer sees the memory effects of all the other
            // consumers before it calls try_dequeue for the last time
        });
    }
    for (int i = 0; i != ProducerCount; ++i) {
        producers[i].join();
